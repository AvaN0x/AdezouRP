...
AddEventHandler('esx_billing:sendBill1', function(playerId, sharedAccountName, label, amount)
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', sharedAccountName, function(account)
		...
		elseif account == nil then
			...
			if xTarget ~= nil then
				MySQL.Async.execute('INSERT INTO billing (identifier, sender, target_type, target, label, amount, date) VALUES (@identifier, @sender, @target_type, @target, @label, @amount, CURRENT_DATE)',
				{
...
RegisterServerEvent('esx_billing:sendBillWithId')
AddEventHandler('esx_billing:sendBillWithId', function(identifier, sharedAccountName, label, amount)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback('esx_billing:getSocietyBills', function(source, cb, societyName)
	MySQL.Async.fetchAll('SELECT * FROM billing WHERE identifier = @identifier', {
		['@identifier'] = societyName
	}, function(result)
		local bills = {}
		for i=1, #result, 1 do
			table.insert(bills, {
