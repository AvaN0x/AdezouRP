Locales["fr"] = {
	["label_count"] = "%s <span>%s</span>",
	["label_count_limit"] = "%s <span>%s/%s</span>",
	["item_detail"] = "Poids : %s kg/unité",
	["title"] = "%s <span>%s/%skg</span>",
	["title_count"] = "x%s %s",

	['label_cash'] = "Liquide : <span style='color: #0cc421'>%s $</span>",
	['label_account_black_money'] = "Argent sale : <span style='color: #eb4034'>%s $</span>",
	['label_dirty_money'] = "Argent sale : <span style='color: #eb4034'>%s $</span>",

	['label_weapon'] = "<span style='color: #eb4034'>%s</span>",
	['weapon_ammo_amount'] = "%s munitions",

	['loadout_weapon_detail'] = "Utilisez l'arme pour récupérer un item stackable",
	['item_weapon_detail'] = "Utilisez l'item pour récupérer une arme utilisable",

	["select_a_player"] = "A qui voulez-vous donner ?",
	["enter_amount"] = "Entrez un montant",
	["use_item"] = "Utiliser",
	["give_item"] = "Donner",
	["drop_item"] = "Jeter",

	["invalid_quantity"] = "Quantité invalide",
	['cash'] = 'Liquide',
	['cash_amount'] = '$%s',

	["target_not_enough_place"] = "La personne n'a pas assez de place dans son inventaire",
	["not_enough_place"] = "Vous n'avez pas assez de place dans votre inventaire",
	["not_enough_place_stock"] = "Il n'y a pas assez de place dans ce coffre",

	["open_inventory"] = "Ouvrir l'inventaire d'un joueur",




	['deposit_stock'] = 'Déposer Stock',
	['take_stock'] = 'Prendre Stock',
	['have_withdrawn'] = 'Vous avez retiré %s',
	['have_deposited'] = 'Vous avez ajouté %s',

}
