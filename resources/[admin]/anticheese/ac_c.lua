...
function isWeaponBlacklisted(model)
	for _, blacklistedWeapon in pairs(weaponblacklist) do
		if model == GetHashKey(blacklistedWeapon) or model == 2982836145 or model == 1746263880 then
			return true
		end
	end
...
--[[ THIS WILL > THAT SPAWN IN WITH GUNS OF COURSE. Restrict AI 
Citizen.CreateThread(function()
	while true do
	Wait(100)
		...
		repeat
			if not IsEntityDead(ped) and not IsPedAPlayer(ped) then
				table.insert(foundpeds, ped)
			end
			finished, ped = FindNextPed(handle)
		until not finished
		EndFindPed(handle)

		for a = 1, #foundpeds do
			local selectedWeapon = GetSelectedPedWeapon(foundpeds[a])
			for b = 1, #weaponblacklist do
				if selectedWeapon == GetHashKey(weaponblacklist[b]) then
					SetEntityHealth(foundpeds[a], 0.0)
				end
			end
		end
