...
CREATE TABLE IF NOT EXISTS `bills` (
    ...
    PRIMARY KEY (`id`),
    CONSTRAINT FK_bills_players_player_from FOREIGN KEY(`player_from`) REFERENCES `players`(`id`) ON DELETE CASCADE,
    CONSTRAINT FK_bills_players_player_to FOREIGN KEY(`player_to`) REFERENCES `players`(`id`) ON DELETE CASCADE,
    CONSTRAINT FK_bills_jobs_job_from FOREIGN KEY(`job_from`) REFERENCES `jobs`(`name`) ON DELETE CASCADE,
    CONSTRAINT FK_bills_jobs_job_to FOREIGN KEY(`job_to`) REFERENCES `jobs`(`name`) ON DELETE CASCADE,
...
CREATE TRIGGER TR_bills_dates BEFORE INSERT ON `bills` FOR EACH ROW
