-------------------------------------------
-------- MADE BY GITHUB.COM/AVAN0X --------
--------------- AvaN0x#6348 ---------------
-------------------------------------------

Config.PropStores = {
    [`prop_vend_soda_01`] = {
        offset = vector3(0.40, -0.45, 0.35),
        Items = {
            { name = "ecola", price = 15, prop = `prop_ecola_can` },
            { name = "sprunk", price = 15, prop = `prop_ld_can_01` },
            { name = "orangotang", price = 15, prop = `prop_orang_can_01` },
            { name = "munkyjuice", price = 15 },
            { name = "junkenergy", price = 15, prop = `sf_prop_sf_can_01a` },
        },
        type = "vending",
    },
    [`prop_vend_soda_02`] = {
        offset = vector3(0.40, -0.45, 0.35),
        Items = {
            { name = "ecola", price = 15, prop = `prop_ecola_can` },
            { name = "sprunk", price = 15, prop = `prop_ld_can_01` },
            { name = "orangotang", price = 15, prop = `prop_orang_can_01` },
            { name = "munkyjuice", price = 15 },
            { name = "junkenergy", price = 15, prop = `sf_prop_sf_can_01a` },
        },
        type = "vending",
    },
    [`sf_prop_sf_vend_drink_01a`] = {
        offset = vector3(0.40, -0.45, 1.25),
        Items = {
            { name = "ecola", price = 15, prop = `prop_ecola_can` },
            { name = "sprunk", price = 15, prop = `prop_ld_can_01` },
            { name = "orangotang", price = 15, prop = `prop_orang_can_01` },
            { name = "munkyjuice", price = 15 },
            { name = "junkenergy", price = 15, prop = `sf_prop_sf_can_01a` },
        },
        type = "vending",
    },
    [`prop_vend_snak_01`] = {
        offset = vector3(0.40, -0.56, 0.35),
        Items = {
            { name = "egochaser", price = 15, prop = `prop_candy_pqs` },
            { name = "meteorite", price = 15, prop = `prop_candy_pqs` },
            { name = "pqs", price = 15, prop = `prop_candy_pqs` },
            { name = "phatchips", price = 12, prop = `v_ret_ml_chips1` },
            { name = "phatchips2", price = 12, prop = `v_ret_ml_chips2` },
            { name = "phatchips3", price = 12, prop = `v_ret_ml_chips3` },
            { name = "phatchips4", price = 12, prop = `v_ret_ml_chips4` },
        },
        type = "vending",
    },
    [`prop_vend_coffe_01`] = {
        offset = vector3(0.24, -0.35, 1.3),
        Items = {
            { name = "coffee", price = 5, useItem = true },
        },
        type = "dispenser",
    },
    [`prop_watercooler`] = {
        offset = vector3(0.0, -0.2, 0.85),
        Items = {
            { name = "watercup_raine", price = 0, prop = `prop_cs_paper_cup`, useItem = true },
        },
        type = "dispenser",
    },
    [`prop_watercooler_dark`] = {
        offset = vector3(0.0, -0.2, 0.85),
        Items = {
            { name = "watercup_raine", price = 0, prop = `prop_cs_paper_cup`, useItem = true },
        },
        type = "dispenser",
    },
    [`prop_vend_water_01`] = {
        offset = vector3(0.18, -0.4, 0.85),
        Items = {
            { name = "waterbottle", price = 15, prop = `prop_ld_flow_bottle` },
            { name = "waterbottle_raine", price = 15, prop = `h4_prop_club_water_bottle` },
        },
        type = "vending",
    },
}
