...
AddEventHandler("vSync:requestSync", function()
    TriggerClientEvent("vSync:updateWeather", -1, CurrentWeather, blackout, iem, snow)
...
RegisterCommand("snow", function(source)
    if source == 0 then
        snow = not snow
        if iem then
            print(_U("snow_enabled"))
        else
            print(_U("snow_disabled"))
        end
    else
        if isAllowedToChange(source) then
            snow = not snow
            if iem then
                TriggerClientEvent("vSync:notify", source, _U("snow_enabledc"))
            else
                TriggerClientEvent("vSync:notify", source, _U("snow_disabledc"))
            end
            TriggerEvent("vSync:requestSync")
        end
    end
