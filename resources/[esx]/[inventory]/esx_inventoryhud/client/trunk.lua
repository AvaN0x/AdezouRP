...
function refreshCarInventory()
    ESX.TriggerServerCallback(
        "esx_trunk:getInventoryV",
        function(inventory)
          local text = trunkData.plate .. ' - ' .. (inventory.weight/1000) .. " / "..(trunkData.max/1000)..' '..' kg'
          local data = {plate = trunkData.plate, max = trunkData.max, myVeh = trunkData.myVeh, text = text}
            setTrunkInventoryData(data, inventory.blackMoney, inventory.items, inventory.weapons)
        end,
        trunkData.plate
      )
