		...
		if #(playerCoords - tPos.xyz) < 30.0 then
			if not PedSpawned then
				...
				if math.random(1, 3) == 1 then
					TriggerServerEvent("esx_phone:sendEmergency", "police", "Une personne suspecte a été aperçue proche de cette position.", true, { ["x"] = (playerCoords.x + math.random(-20, 20)), ["y"] = (playerCoords.y + math.random(-20, 20)), ["z"] = playerCoords.z })
				end
			end
