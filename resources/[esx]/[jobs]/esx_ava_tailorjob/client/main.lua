...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_ava_tailorjob:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_ava_tailorjob:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_ava_tailorjob:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_ava_tailorjob:putStockItems', itemName, count)
						end
...
AddEventHandler('esx_ava_tailorjob:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_ava_tailorjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)
	...
	while true do
		...
		if (PlayerData.job ~= nil and PlayerData.job.name == Config.JobName) or (PlayerData.job2 ~= nil and PlayerData.job2.name == Config.JobName) then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_ava_tailorjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_ava_tailorjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
		...
		if (PlayerData.job ~= nil and PlayerData.job.name == Config.JobName) or (PlayerData.job2 ~= nil and PlayerData.job2.name == Config.JobName) then
			...
			if nearbyObject and nearbyObject.obj and IsPedOnFoot(playerPed) then
				...
				if IsControlJustReleased(0, Keys['E']) and not isPickingUp then
					...
					ESX.TriggerServerCallback('esx_ava_tailorjob:canPickUp', function(canPickUp)
						...
						if canPickUp then
							...
							for i=1, #nearbyObject.items, 1 do
								TriggerServerEvent('esx_ava_tailorjob:pickUp', nearbyObject.items[i])
							end
						else
...
function Process(v)
	...
	ESX.TriggerServerCallback('esx_ava_tailorjob:canprocess', function(canProcess)
		isProcessing = true
		if canProcess then
			TriggerServerEvent('esx_ava_tailorjob:process', v)
			...
			while timeLeft - 2 > 0 do
				...
				if GetDistanceBetweenCoords(GetEntityCoords(playerPed), v.Pos.x, v.Pos.y, v.Pos.z, false) > 2 then
					TriggerServerEvent('esx_ava_tailorjob:cancelProcessing')
					break
				end
			end
			Citizen.Wait(2000)
			...
			for k,v in pairs(Config.SellZones) do
				if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < 2 then
					...
					if IsControlJustReleased(0, Keys['E']) then
						ESX.TriggerServerCallback('esx_ava_tailorjob:GetSellElements', function(elements)
							ESX.UI.Menu.CloseAll()
							isSelling = true
							ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'job_seller',{ title = "Acheteur "..Config.LabelName, align = 'left', elements = elements },
								function(data,menu) 
									...
									if tonumber(count) > tonumber(data.current.owned) then 
										...
										TriggerServerEvent('esx_ava_tailorjob:SellItems',data.current.name,data.current.price,count)
										menu.close()
										isSelling = false
										Citizen.Wait(1500)
									end
		...
		if (PlayerData.job ~= nil and PlayerData.job.name == Config.JobName) or (PlayerData.job2 ~= nil and PlayerData.job2.name == Config.JobName) then
			...
			for k,v in pairs(Config.BuyZones) do
				if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < 2 then
					...
					if IsControlJustReleased(0, Keys['E']) then
						ESX.TriggerServerCallback('esx_ava_tailorjob:GetBuyElements', function(elements)
							ESX.UI.Menu.CloseAll()
							isBuying = true
							ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'job_buyer',{ title = "Vendeur pour "..Config.LabelName, align = 'left', elements = elements },
								function(data,menu) 
									...
									TriggerServerEvent('esx_ava_tailorjob:BuyItems',data.current.name,data.current.price,count)
									isBuying = false
									Citizen.Wait(1500)
								end,
								function(data,menu)
