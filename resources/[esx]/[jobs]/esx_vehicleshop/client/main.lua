...
function OpenShopMenu()
	...
	for i=1, #Categories, 1 do
		-- if Categories[i].name == "compacts" or 
		-- 	Categories[i].name == "coupes" or 
		-- 	Categories[i].name == "motorcycles" or 
		-- 	Categories[i].name == "muscle" or 
		-- 	Categories[i].name == "offroad" or 
		-- 	Categories[i].name == "sedans" or 
		-- 	Categories[i].name == "sports" or 
		-- 	Categories[i].name == "sportsclassics" or
		-- 	Categories[i].name == "super" or 
		-- 	Categories[i].name == "suvs" or  
		-- 	Categories[i].name == "vans" or 
		if not string.match(Categories[i].name, '^society_.*$') or -- on récupère les catégories normales
			...
			for j=1, #categoryVehicles, 1 do
				...
				table.insert(options, ('%s <span style="color:green;">%s</span>'):format(vehicle.name, _U('generic_shopitem', (vehicle.price))))
			end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				if Config.EnablePlayerManagement then
					...
					if Config.EnableSocietyOwnedVehicles and playerData.job.grade_name == 'boss' and string.match(data.current.name, '^society_.*$')then
						local elements = {
							-- {label = _U('staff_type'),   value = 'personnal'}, -- on ne peut pas acheter un véhicule d'entreprise en perso
							-- {label = _U('society_type'), value = 'society'},
							-- {label = _U('society_type2'), value = 'society2'}
						}
						...
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm_buy_type', {
							...
							-- 	{label = _U('society_type'), value = 'society'},
							-- 	{label = _U('society_type'), value = 'society2'}
							-- }
							elements = elements
						...
						ESX.TriggerServerCallback('esx_vehicleshop:buyVehicle', function (hasEnoughMoney)
							if hasEnoughMoney then
								ESX.TriggerServerCallback('eden_garage:getParkingInfos', function(parkingInfos)									
		...
		elseif action == 'set_vehicle_owner_sell' then
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				if Config.EnableOwnedVehicles then
					...
					ESX.ShowNotification(_U('vehicle_set_owned', vehicleProps.plate))
					...
					ESX.ShowNotification(_U('vehicle_sold_to', vehicleProps.plate))
					TriggerServerEvent('esx_vehiclelock:registerkey', vehicleProps.plate, GetPlayerServerId(closestPlayer))
				end
			end
