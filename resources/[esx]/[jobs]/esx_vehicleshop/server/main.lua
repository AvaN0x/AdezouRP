...
MySQL.ready(function()
	...
	Categories = MySQL.Sync.fetchAll('SELECT * FROM vehicle_categories UNION SELECT addon_account.name, addon_account.label FROM vehicles_society JOIN addon_account ON society = addon_account.name')
	-- local vehicles = MySQL.Sync.fetchAll('SELECT * FROM vehicles')
	local vehicles = MySQL.Sync.fetchAll('SELECT * FROM vehicles UNION SELECT * FROM vehicles_society')
...
AddEventHandler('esx_vehicleshop:addToList', function(target, model, plate)
	...
	-- TODO utiliser ça pour mettre nom et prenom
	local result = MySQL.Sync.fetchAll('SELECT firstname, lastname FROM users WHERE identifier = @identifier', {
		['@identifier'] = xPlayer.identifier
	})
	local xPlayerName = (result[1].firstname or 'FIRSTNAME') .. ' ' .. (result[1].lastname or 'LASTNAME')
	result = MySQL.Sync.fetchAll('SELECT firstname, lastname FROM users WHERE identifier = @identifier', {
		['@identifier'] = xTarget.identifier
	})
	local xTargetName = (result[1].firstname or 'FIRSTNAME') .. ' ' .. (result[1].lastname or 'LASTNAME')
