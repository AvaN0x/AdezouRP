...
function terminateGame(reasonForTerminating, failed)
    if failed then
        Scaleform.ShowMessage(Wrapper._('terminate_failed'), reasonForTerminating, 5)
        playSound(Config.Sounds.failedMission)
    else
        Scaleform.ShowMessage()
...
function handlePatientDropOff()
    ...
    if #gameData.peds == 0 then
        ...
        local moneyPerLevel = Config.Formulas.moneyPerLevel(gameData.level);
    ...
    for index, ped in pairs(gameData.peds) do
        ...
        if distanceFromPed <= Config.PedPickupDistance then
            ...
            elseif #gameData.peds == 0 then
                ...
                hospitalBlip = AddBlipForCoord(gameData.hospitalLocation.x, gameData.hospitalLocation.y, gameData.hospitalLocation.z)
...
function addTime(timeToAdd)
    ...
    if timeToAdd > 0 then
        ESX.ShowNotification(Wrapper._('time_added', timeToAdd))
        playSound(Config.Sounds.timeAdded)
