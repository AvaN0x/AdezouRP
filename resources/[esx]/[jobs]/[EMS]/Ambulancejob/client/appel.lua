...
AddEventHandler("AppelemsTropBien", function(coords, id)
	...
	AppelCoords = coords
	AppelID = id
	ESX.ShowAdvancedNotification("ems", "~b~Demande d'EMS", "Quelqu'un à besoin d'un EMS !\n~g~Y~w~ pour prendre l'appel\n~r~X~w~ pour refuser", "CHAR_CALL911", 8)
end)
     ...
     while true do
		...
		if AppelEnAttente then
			...
			elseif IsControlJustPressed(1, 73) and AppelEnAttente then
				ESX.ShowAdvancedNotification("ems", "~b~Demande d'EMS", "Vous avez refusé l'appel.", "CHAR_CALL911", 8)
		...
		if IsControlJustPressed(1, 246) and AppelDejaPris == true then
			ESX.ShowAdvancedNotification("ems", "~b~Demande d'EMS", "L'appel à déja été pris, désolé.", "CHAR_CALL911", 8)
		end
     end
end)
...
AddEventHandler("emsAppelPris", function(Xid, XAppelCoords)
	ESX.ShowAdvancedNotification("ems", "~b~Demande d'EMS", "Vous avez pris l'appel, suivez la route GPS.", "CHAR_CALL911", 8)   
	...
	-- Ajout de la route
     ...
     while rea do
          if GetDistanceBetweenCoords(XAcoords, GetEntityCoords(GetPlayerPed(-1))) < 10.0 then
               ...
               Draw3DText(XAcoords.x, XAcoords.y, XAcoords.z, "~g~Une personne a besoin d'une réanimation ici ...\n~b~[E]~w~Pour la réanimer.", 4, 0.1, 0.1)
...
Citizen.CreateThread( function()	
	while true do
		Wait(10)
		if enService then
			DrawRect(0.888, 0.084, 0.196, 0.116, 0, 0, 0, 30)
			DrawAdvancedText(0.984, 0.044, 0.008, 0.0028, 0.4, "Dernière prise d'appel:", 0, 191, 255, 255, 6, 0)
			DrawAdvancedText(0.988, 0.066, 0.005, 0.0028, 0.4, "~b~"..NomAppel.." ~w~a pris le dernier appel EMS", 255, 255, 255, 255, 6, 0)
			DrawAdvancedText(0.984, 0.104, 0.008, 0.0028, 0.4, "Total d'appels pris en compte", 0, 191, 255, 255, 6, 0)
			DrawAdvancedText(0.988, 0.124, 0.005, 0.0028, 0.4, AppelTotal, 255, 255, 255, 255, 6, 0)
