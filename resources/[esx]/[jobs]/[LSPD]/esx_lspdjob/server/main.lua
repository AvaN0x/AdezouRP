...
AddEventHandler('esx_policejob:getStockItem', function(itemName, count)
	...
	TriggerEvent('esx_ava_inventories:getSharedInventory', 'society_police', function(inventory)
...
ESX.RegisterServerCallback('esx_policejob:getVehicleInfos', function(source, cb, plate)
	...
	MySQL.Async.fetchAll('SELECT owner FROM owned_vehicles WHERE plate = @plate', {
		...
		if result[1] and string.match(result[1].owner, '^society_.*$') then
			MySQL.Async.fetchAll('SELECT label FROM addon_account WHERE name = @name',  {
				['@name'] = result[1].owner
			}, function(result2)

				retrivedInfo.owner = result2[1].label
...
ESX.RegisterServerCallback('esx_policejob:getArmoryWeapons', function(source, cb, weaponsJSON)
	TriggerEvent('esx_datastore:getSharedDataStore', 'society_police', function(store)
		local weapons = store.get(weaponsJSON)
...
ESX.RegisterServerCallback('esx_policejob:addArmoryWeapon', function(source, cb, weaponName, removeWeapon, weaponsJSON)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	TriggerEvent('esx_datastore:getSharedDataStore', 'society_police', function(store)
		...
		store.set(weaponsJSON, weapons)
		cb()
	end)
end)

ESX.RegisterServerCallback('esx_policejob:removeArmoryWeapon', function(source, cb, weaponName, weaponsJSON)
	local xPlayer = ESX.GetPlayerFromId(source)
