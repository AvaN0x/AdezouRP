			...
			if PlayerData.job ~= nil and PlayerData.job.name == "lspd" then
				if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance) then
					...
					if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < 1.5 then
						DrawText3D(v.Pos.x, v.Pos.y, v.Pos.z+0.9, "~w~[~g~E~w~] Utiliser les caméras.", 0.80)
					end
				end
			end
		end
			...
			if Config.Hacking then
				if PlayerData.job ~= nil and PlayerData.job.name == "lspd" then
					...
					if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance) then
						...
						if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < 1.5 then
							DrawText3D(v.Pos.x, v.Pos.y, v.Pos.z+0.9, "~w~[~g~E~w~] Pirater les caméras.", 0.80)
						end
					end
				end
			end
			...
			if Config.Hacking then
				if PlayerData.job ~= nil and PlayerData.job.name == "lspd" then
					if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance) then
						...
						if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < 1.5 then
							DrawText3D(v.Pos.x, v.Pos.y, v.Pos.z+0.9, "~w~[~g~E~w~] Supprimer le virus.", 0.80)
						end
					end
				end
			end
		...
		if IsControlJustReleased(0, Keys['E']) and CurrentAction == 'cameras' then
			...
			if not menuopen then
				...
				local elements = {
					...
					{label = _U('city_menu_selection'), value = 'citymenu'}
				}
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
				{
					...
					elseif data.current.value == 'citymenu' then
						...
						if policeHacked then
							...
							citycamera = true
							blockbuttons = true
							local firstCamx = Config.Locations[1].cityCam[1].x
							local firstCamy = Config.Locations[1].cityCam[1].y
							local firstCamz = Config.Locations[1].cityCam[1].z
							local firstCamr = Config.Locations[1].cityCam[1].r
							...
							SendNUIMessage({
								type  = "enablecam",
								label = Config.Locations[1].cityCam[1].label,
								box   = Config.Locations[1].cityCamLabel.label
							})
		...
		if createdCamera ~= 0 then
			...
			if IsControlJustPressed(0, Keys["LEFT"]) then
				...
				elseif policecamera then
					...
					ChangeSecurityCamera(newCamx, newCamy, newCamz, newCamr)
					currentCameraIndexIndex = newCamIndex
				elseif citycamera then
					local newCamIndex

					if currentCameraIndexIndex == 1 then
						newCamIndex = #Config.Locations[currentCameraIndex].cityCam
					else
						newCamIndex = currentCameraIndexIndex - 1
					end

					local newCamx = Config.Locations[currentCameraIndex].cityCam[newCamIndex].x
					local newCamy = Config.Locations[currentCameraIndex].cityCam[newCamIndex].y
					local newCamz = Config.Locations[currentCameraIndex].cityCam[newCamIndex].z
					local newCamr = Config.Locations[currentCameraIndex].cityCam[newCamIndex].r

					SetFocusArea(newCamx, newCamy, newCamz, newCamx, newCamy, newCamz)
					SendNUIMessage({
						type = "updatecam",
						label = Config.Locations[currentCameraIndex].cityCam[newCamIndex].label
					})
			...
			if IsControlJustPressed(0, Keys["RIGHT"]) then
				...
				elseif citycamera then
					...
					if currentCameraIndexIndex == #Config.Locations[currentCameraIndex].cityCam then
						newCamIndex = 1
					else
						newCamIndex = currentCameraIndexIndex + 1
					end
			...
			elseif Config.Locations[currentCameraIndex].cityCam[currentCameraIndexIndex].canRotate then
				local getCameraRot = GetCamRot(createdCamera, 2)
...
function mycbpolice(success, timeRemaining)
	if success then
		...
		--TriggerServerEvent('esx_phone:send', 'police', 'ALARME DE POLICE: QUELQU\'UN ESSAYE DE PIRATER NOS ORDINATEURS. VENEZ À LA STATION RAPIDEMENT.', false, false)
	end
end
