		...
		function(data, menu)
			if not HasPedGotWeapon(PlayerPedId(), GetHashKey(data.current.value), false) then
				...
				if DoesEntityExist(closestPed) and closestPedDst <= 5.0 then
					...
					Citizen.Wait(1100)
					GiveWeaponToPed(closestPed, GetHashKey(data.current.value), 1, false, true)
					SetCurrentPedWeapon(closestPed, GetHashKey(data.current.value), true)
					TaskPlayAnim(PlayerPedId(), animLib, anim .. "_on_counter", 1.0, -1.0, 1.0, 0, 0, 0, 0, 0)
					Citizen.Wait(3100)
					RemoveWeaponFromPed(closestPed, GetHashKey(data.current.value))
					Citizen.Wait(15)
					GiveWeaponToPed(PlayerPedId(), GetHashKey(data.current.value), Config.ReceiveAmmo, false, true)
					SetCurrentPedWeapon(PlayerPedId(), GetHashKey(data.current.value), true)
