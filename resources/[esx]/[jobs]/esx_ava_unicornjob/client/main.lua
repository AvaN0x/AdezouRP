...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_ava_unicornjob:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_ava_unicornjob:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_ava_unicornjob:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_ava_unicornjob:putStockItems', itemName, count)
						end
...
AddEventHandler('esx_ava_unicornjob:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_ava_unicornjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)
	...
	while true do
		...
		if (PlayerData.job ~= nil and PlayerData.job.name == Config.JobName) or (PlayerData.job2 ~= nil and PlayerData.job2.name == Config.JobName) then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_ava_unicornjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_ava_unicornjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
			...
			for k,v in pairs(Config.BuyZones) do
				if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < 2 then
					...
					if IsControlJustReleased(0, Keys['E']) then
						ESX.TriggerServerCallback('esx_ava_unicornjob:GetBuyElements', function(elements)
							ESX.UI.Menu.CloseAll()
							isBuying = true
