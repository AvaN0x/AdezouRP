...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_ava_nightclubjob:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_ava_nightclubjob:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_ava_nightclubjob:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_ava_nightclubjob:putStockItems', itemName, count)
						end
...
AddEventHandler('esx_ava_nightclubjob:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_ava_nightclubjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.JobBlip.Pos.x, Config.Zones.JobBlip.Pos.y, Config.Zones.JobBlip.Pos.z)
	...
	while true do
		...
		if (PlayerData.job ~= nil and PlayerData.job.name == Config.JobName) or (PlayerData.job2 ~= nil and PlayerData.job2.name == Config.JobName) then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_ava_nightclubjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_ava_nightclubjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
			...
			for k,v in pairs(Config.BuyZones) do
				if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < 2 then
					...
					if IsControlJustReleased(0, Keys['E']) then
						ESX.TriggerServerCallback('esx_ava_nightclubjob:GetBuyElements', function(elements)
							ESX.UI.Menu.CloseAll()
							isBuying = true
							ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'job_buyer',{ title = "Vendeur pour "..Config.LabelName, align = 'left', elements = elements },
								function(data,menu) 
									...
									TriggerServerEvent('esx_ava_nightclubjob:BuyItems',data.current.name,data.current.price,count)
									isBuying = false
									Citizen.Wait(1500)
								end,
								function(data,menu)
