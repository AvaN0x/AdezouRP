...
if closestPlayer == -1 or closestDistance > 2.0 then 
  ...
  if not HaveBagOnHead then
    ...
    -- ESX.ShowNotification('~g~Vous mettez un Sac-tête à un citoyen ~w~' .. #GetPlayerName(closestPlayer))
    ESX.ShowNotification('~g~Vous mettez un Sac-tête à un citoyen')
    TriggerServerEvent('esx_worek:closest')
  else
    ESX.ShowNotification('~r~Ce joueur à déjà un Sac-tête d\'équipé')
  end
end
...
AddEventHandler('esx_worek:zdejmijc', function(gracz)
    ESX.ShowNotification('~g~Quelqu\'un vous a enlevé le Sac-tête!')
    DeleteEntity(Worek)
    SetEntityAsNoLongerNeeded(Worek)
    SendNUIMessage({type = 'closeAll'})
    HaveBagOnHead = false
