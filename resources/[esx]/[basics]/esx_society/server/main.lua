...
AddEventHandler('esx_society:withdrawMoney', function(society, amount)
  ...
  TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
    ...
    if amount > 0 and account.money >= amount then
      ...
      TriggerEvent('esx_society:saveData',xPlayer,"withdraw",society,amount,new)

      account.removeMoney(amount)
      xPlayer.addMoney(amount)

      TriggerEvent('esx_avan0x:logTransaction', society.account, society.account, xPlayer.identifier, 'money', "withdraw", amount)
...
RegisterServerEvent('esx_society:withdrawMoneyDirty')
AddEventHandler('esx_society:withdrawMoneyDirty', function(society, amount)
  ...
  TriggerEvent('esx_addonaccount:getSharedAccount', society.account.."_black", function(account)
    if amount > 0 and account.money >= amount then
      local new = account.money-amount
      TriggerEvent('esx_society:saveData',xPlayer,"withdraw_black",society,amount,new)
...
AddEventHandler('esx_society:depositMoney', function(society, amount)
  ...
  if amount > 0 and xPlayer.get('money') >= amount then
    TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
      local new = account.money+amount
      TriggerEvent('esx_society:saveData',xPlayer,"deposit",society,amount,new)
      xPlayer.removeMoney(amount)
      account.addMoney(amount)
    end)

    TriggerEvent('esx_avan0x:logTransaction', xPlayer.identifier, 'money', society.account, society.account, "deposit", amount)
...
AddEventHandler('esx_society:depositMoneyDirty', function(_society, amount)
  ...
  if amount > 0 and xPlayer.getAccount('black_money').money >= amount then
    TriggerEvent('esx_addonaccount:getSharedAccount', society.account.."_black", function(account)
      local new = account.money+amount
      TriggerEvent('esx_society:saveData',xPlayer,"deposit_black",society,amount,new)
...
RegisterServerEvent('esx_society:saveData')
AddEventHandler('esx_society:saveData', function(xPlayer,type,society,amount,new)
  local id = xPlayer.identifier
  MySQL.Async.execute('INSERT INTO account_logs (id,type,society,amount,new) VALUES (@id,@type,@society,@amount,@new)',
  {
    ['@id'] = id,
    ['@type'] = type,
...
ESX.RegisterServerCallback('esx_society:getSocietyDirtyMoney', function(source, cb, societyName)
