...
function OpenBossMenu(society, close, options)
    ...
    if options.withdrawDirty then
        table.insert(elements, {label = _U('withdraw_society_money_dirty'), value = 'withdraw_society_money_dirty'})
    end

    if options.depositDirty then
        table.insert(elements, {label = _U('deposit_society_money_dirty'), value = 'deposit_money_dirty'})
    end
    ...
    if options.bills then
      table.insert(elements, {label = 'Factures', value = 'bills'})
    end    
  ...
  ---SECONDJOB INCLUDED
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'boss_actions_' .. society,
    {
        ...
        elseif data.current.value == 'withdraw_society_money_dirty' then

            ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'withdraw_society_money_amount_dirty_' .. society,
            {
                title = _U('withdraw_amount')
            }, function(data, menu)
                ...
                if amount == nil then
                    ...
                    menu.close()
                    TriggerServerEvent('esx_society:withdrawMoneyDirty', society, amount)
                end

            end, function(data, menu)
                menu.close()
            end)
        elseif data.current.value == 'deposit_money_dirty' then

            ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'deposit_money_amount_dirty_' .. society,
            {
                ...
                if amount == nil then
                    ...
                    menu.close()
                    TriggerServerEvent('esx_society:depositMoneyDirty', society, amount)
                end
        ...
        elseif data.current.value == 'bills' then
          ESX.TriggerServerCallback('esx_billing:getSocietyBills', function(result)

            local elements = {}
    
            for i=1, #result, 1 do
              table.insert(elements, {label = result[i].label..' $'..result[i].amount, value = result[i].id})
            end
    
    
            ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'bills_society_'..society, 
            {
              title    = 'Factures',
              align    = 'left',
              elements = elements
...
function OpenManageGradesMenu(society)
  ...
  ESX.TriggerServerCallback('esx_society:getJob', function(job)
    ...
    for i=1, #job.grades, 1 do
      if tostring(job.grades[i].name) ~= "interim" then
        local gradeLabel = (job.grades[i].label == '' and job.label or job.grades[i].label)
	...
	while true do
		...
		if (PlayerData.job ~= nil and PlayerData.job.grade_name == 'boss') or (PlayerData.job2 ~= nil and PlayerData.job2.grade_name == 'boss') then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_society:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_society:hasExitedMarker', LastZone)
			end
    -- else
    --   Citizen.Wait(5000)
    end
...
AddEventHandler('esx_society:hasEnteredMarker', function(zone)
	if (PlayerData.job ~= nil and PlayerData.job.grade_name == 'boss') or (PlayerData.job2 ~= nil and PlayerData.job2.grade_name == 'boss') then
			
		if zone == 'ActionBank' then
			CurrentAction     = 'open_action_bank_menu'
			CurrentActionMsg  = _U('open_action_bank')
			CurrentActionData = {}
		end
	end
end)

AddEventHandler('esx_society:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  38) and ((PlayerData.job ~= nil and PlayerData.job.grade_name == 'boss') or (PlayerData.job2 ~= nil and PlayerData.job2.grade_name == 'boss')) then
        if CurrentAction == 'open_action_bank_menu' then
          OpenBankMenu()
          -- OpenBossMenu(PlayerData.job.name, function(data, menu) 
          --   menu.close()
          -- end, {})	
				end
...
function OpenBankMenu()
  ...
  if PlayerData.job ~= nil and PlayerData.job.grade_name == 'boss' then
    table.insert(elements, {label = PlayerData.job.label, value = PlayerData.job.name})
  end
  if PlayerData.job2 ~= nil and PlayerData.job2.grade_name == 'boss' then
    table.insert(elements, {label = PlayerData.job2.label, value = PlayerData.job2.name})
  end

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'state_cloakroom',
	{
		title    = _U('action_bank'),
		align    = 'left',
		elements = elements
	}, function(data, menu)
    OpenBossMenu(data.current.value, function(data2, menu2) 
      menu2.close()
    end, {
      withdraw  = true,
      deposit   = true,
